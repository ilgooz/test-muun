...
# docker buildx build --tag test --file .pi/Dockerfile --platform linux/arm64 --cache-from test:cache --cache-to test:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar test
...
sudo bash -c "echo test > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/test.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/test.img
